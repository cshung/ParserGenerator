...
namespace ParsingLab2
{
    ...
    class ParserGenerator
    {
        ...
        {
            bool dumpFirstSet = true;
            bool dumpFollowSet = true;
            bool dumpCanonicalSet = true;
            bool dumpTables = true;
        ...
        private void DumpCanonicalSets()
        {
            ...
            {
                Console.WriteLine("-----------------------------------");
                Console.WriteLine("Canonical Set #{0}", i++);
                foreach (var item in canonicalSet)
                {
                    var itemProduction = this.rewrittenGrammar[item.Production];
            ...
            foreach (var transition in this.canonicalSetTransitions)
            {
                Console.WriteLine("{0} -{1}-> {2}", transition.From, ToSymbol(transition.Symbol).DisplayName, transition.To);
            }
        }
